...
public class HotelSystem {
    ...
    public synchronized Reservation booking(Guest guest, Room room, LocalDate checkInDate, LocalDate checkOutDate) {
    ...
    public synchronized void checkOut(String reservationId, Payment payment) {
        ...
        if (reservation != null && reservation.getStatus() == ReservationStatus.CONFIRMED) {
            ...
            if (payment.processPayment(amount)) {
                room.checkOut();
                reservations.remove(reservationId);
                System.out.println("Room checkout successfully done!");
            } else throw new IllegalArgumentException("Payment failed");
        } else throw new IllegalArgumentException("Invalid reservation or reservation not confirmed");
    }
}
